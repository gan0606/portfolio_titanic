...
def main():
    ...
    if chosen_menu == "分析概要":
        ...
        st.write("訓練データには、891人の乗客情報 (生存フラグ、名前、年齢、性別、船室の料金、乗船港など)が含まれます。")
    ...
    # データセットの概要
        ...
        st.subheader("分析目的")
        st.write("・乗客の属性情報 (年齢、性別、船室の料金など) を基に、生存確率を予測すること")
        ...
        st.write("ROC AUCスコアについて")
        st.write("機械学習モデルの分類精度を評価する指標の一つです。諸説ありますが、一般的に0.8以上だと高い精度とされています。")
        st.write("""
                 生存確率の分布から生存確率が0から1までの間に広く分布していることは、
                 モデルがあるデータポイントの生存する可能性を様々に評価していることを示唆しています。
                 """)
        st.write("これは、モデルがデータに適切に適合し、高い汎化性能を持つ可能性があることを示唆しています。")
    elif chosen_menu == "生存要因":
        st.subheader("SHAP分析の結果")
        st.write("SHAP値とは、乗客の属性情報 (年齢、性別、船室の料金など)がモデルの予測にどの程度貢献しているかを数字で表しています。")

        # shap分析の結果を表示
        st.write("SHAP値は、各特徴量が予測に与える影響度を表す指標です。下記のSHAP Tree図は、中央線が0を表しており、左側が死亡、右側が生存を表します。")
        st.write("例えば、Age(年齢)は濃いピンクが左側に大きく伸びていることから年齢が高くなるほど、生存確率が低くなることを示しています。")
        st.write("また、Family(同行者数)も濃いピンクが左側に大きく伸びていることから同行者数が多いほど生存確率が低くなることを示しています。")
        st.write("一方で、Fare(運賃)は濃いピンクが右側に大きく伸びていることから運賃が大きいほど生存確率が高くなることを示しています。")
        st.write("[注意]")
        st.write("SHAP値は、定量的な属性情報の数字的な強さを表すことができますが、定性的な属性情報の数字的な強さを表すことはできません。")
        st.write("""
                 例えば、性別は定性的な属性情報であり、生存予測において重要な役割を果たしたことが確かです。
                 しかし、女性が男性より生存確率が高い可能性も低い可能性も考えられます。
                 """)
        # shapの可視化
        ...
        st.write("・高い運賃を支払った方が優先的に助けられたと考えられます。")
